...
var instances = make(map[string]*dashboard.InstanceDesc)
...
func main() {
	...
	flag.Parse()
	http.HandleFunc("/api/listDir", listDir)
	http.HandleFunc("/api/import", importInstance)
	http.HandleFunc("/api/updateConfig", updateConfig)
	http.HandleFunc("/api/list", listInstance)
	http.HandleFunc("/api/create", initInstance)
	http.HandleFunc("/api/restart", restartInstance)
	http.HandleFunc("/api/shutdown", shutdownInstance)
	...
	if importPath := r.URL.Query().Get("path"); importPath != "" {
		...
		if e = err; err == nil {
			...
			if hasMain && hasConfig && hasMod && hasRestart {
				...
				instances[name] = &dashboard.InstanceDesc{
					Name:    name,
					Path:    importPath,
					Plugins: nil,
		...
		if err = os.MkdirAll(instancesDir, os.FileMode(0777)); err == nil {
			if f, err := os.Open(instancesDir); err != nil {
				...
				for _, configFile := range cs {
					des := new(dashboard.InstanceDesc)
...
func website(w http.ResponseWriter, r *http.Request) {
	...
	if f, err := ioutil.ReadFile(path.Join(path.Dir(currentFilePath), "dashboard/dist", filePath)); err == nil {
...
func initInstance(w http.ResponseWriter, r *http.Request) {
	instanceDesc := new(dashboard.InstanceDesc)

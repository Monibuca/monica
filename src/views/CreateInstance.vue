<template>
  <a-layout>
    ...
    <a-layout-footer>
      ...
      >
      <template v-else>
        <a-button :loading="creating" @click="startCreate"> 开始创建 </a-button>
      </template>
...
ListenAddr = "192.168.1.120:5060"`);
    ...
    return {
      ...
      currentStep,
      next() {
        if (step.value != "4") step.value = (+step.value + 1).toString();
      },
      startCreate() {
        creating.value = true;
